    ...
    for i in range(0,len(codes),500):
        ...
        try:
            ...
            for line in text.split(';'):
                ...
                try: price=float(z[3] or 0); pct=float(z[32] or 0); amount=float(z[37] or 0)
                except Exception: continue
                if price>0 and code: out.append({'code':code,'name':name,'price':price,'pct':pct,'amount':amount,'turnover':0.0})
        except Exception as e: print('tencent chunk failed',i,repr(e))
...
def main():
    ...
    if not clean: raise RuntimeError('all snapshot sources failed: '+ ' | '.join(errors))
    ...
    data={'updated_at':now,'timezone':'Asia/Shanghai','market':{'stock_count':len(clean),'limit_up':len(limitups),'limit_down':len(limitdowns),'avg_pct':round(sum(x['pct'] for x in clean)/len(clean),2),'total_amount':total_amount},'radar':{'lianban':[x for x in records if any('连板' in t for t in x['tags'])][:30],'second_wave':[x for x in records if '二波启动' in x['tags']][:30],'main_rise':[x for x in records if '主升趋势' in x['tags']][:30],'trend':records[:60]},'source':source,'health':{'universe':len(clean),'deep_scan':len(candidates),'records':len(records),'errors':errors},'note':'无Token多源版本。快照优先东方财富，失败切换腾讯；失败时保留上一份成功数据。'}
